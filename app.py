...
def home():
    return jsonify({
        "status": "ok",
        "message": "API is running. Use /join"
    })
...
async def perform_emote(team_code, uids, emote_id):
    try:
        ...
        # ❗❗❗ IMPORTANT ❗❗❗
        # YAHAN tumhara REAL Free Fire emote logic hoga
        # Example:
        # - socket connect
        # - packet send
        # - websocket / bot logic
        #
        # Abhi sirf demo delay:
...
def join_team():
    ...
    # cold start safety
    if not loop_ready or loop is None:
        return jsonify({
            "status": "error",
            "message": "Server starting, try again in 2 seconds"
        }), 503

    try:
        team_code = request.args.get('tc')
        uid1 = request.args.get('uid1')  # compulsory
        uid2 = request.args.get('uid2')  # optional
        uid3 = request.args.get('uid3')  # optional
        emote_id_str = request.args.get('emote_id')

        # validation
        if not team_code or not emote_id_str or not uid1:
            return jsonify({
                "status": "error",
                "message": "tc, emote_id and uid1 are required"
            }), 400
        ...
        # collect uids (max 3)
        uids = [uid1, uid2, uid3]
        uids = [u for u in uids if u and u != "0"]
...
async def MaiiiinE():
    ...
    Uid, Pw = '4377924645', 'E38B33279592B66C345E39307987563D73051170E0291D78E09AD1F39A708281'
